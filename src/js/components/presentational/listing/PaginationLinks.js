...
class PaginationLinks extends React.Component {
	render() {
		...
		if(currentPage > 1) {
			const link = <Link to={'/Opinnaytetyo_spring_react/discover/' + previousPage}>
		...
		if(currentPage > 3) {
			const link = <Link to='/Opinnaytetyo_spring_react/discover'>
				<button>1</button>
			</Link>
			links.push(link);
		}
		...
		for(let i = -2; i < 3; i++) {
			...
			if(page > 0 && page < 30) {
				const link = <Link to={'/Opinnaytetyo_spring_react/discover/' + page}>
		...
		if(currentPage < 28) {
			const link = <Link to='/Opinnaytetyo_spring_react/discover/30'>
				<button>30</button>
			</Link>
			links.push(link);
		}
